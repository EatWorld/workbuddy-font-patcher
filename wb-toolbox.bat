...
echo.
echo 注意：字体名要填【浏览器用名】，即工具箱 [5] 里带 ★ 的那个。
echo       系统里常有两套名字，比如「仓耳今楷03-W04.ttf」：
echo         ★ 仓耳今楷03      ← 浏览器用名，填这个
echo         ? 仓耳今楷03 W04  ← 注册表名，浏览器认不出
echo       填错了不报错，只会静默回退默认字体，看着像改了其实没生效。
echo       不确定就先选 [5] 查，认准带 ★ 的名字。
echo.
pause
