...
| `font-resolver.js` | 查字体**真实家族名**：字体名填错会静默回退、看着改了其实没生效。菜单一 [5] 就是它 |
...
- **新增 `font-resolver.js`（工具箱 [5]「查字体真实名字」）**：修复一个隐蔽缺陷——CSS 里填的字体名必须和字体文件的**真实家族名**一字不差。例如安装 `仓耳今楷03-W04.ttf` 后，真实名字是「仓耳今楷03 W04」而不是「仓耳今楷03」；填错**不会报错**，浏览器只会静默回退到默认字体，表现为"看着改了其实没生效"。该工具扫描系统字体（`C:\Windows\Fonts` + 用户字体目录），解析 TTF name 表给出真实家族名，支持关键词模糊匹配。
- 工具箱菜单从 4 项扩到 5 项，改字体入口处加了对"真实家族名"的醒目提示。
